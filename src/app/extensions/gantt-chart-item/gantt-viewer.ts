...
// import GanttChart, { dxGanttChartOptions } from "devextreme/viz/gantt_chart";
...
export class GanttChartItem extends CustomItemViewer {

    ganttChartSettings;
    ganttChartViewer;
    selectionValues: Array<any>
    exportingImage: HTMLImageElement;
    ganttChartContainer: HTMLElement;
    element;
    series = [];
    constructor(model, container, options) {
        super(model, container, options);

        this.ganttChartSettings = undefined;
        this.ganttChartViewer = null;
    ...
    renderContent(element: dxElement, changeExisting) {
        let htmlElement: HTMLElement = <HTMLElement>(<any>element);
        this.element = element;
        var data = this._getDataSource();
        if (!this._ensureganttChartLibrary(htmlElement))
            return;
        if (!!data) {
            if (!changeExisting || !this.ganttChartViewer) {
                while (htmlElement.firstChild)
                    htmlElement.removeChild(htmlElement.firstChild);

                this.ganttChartContainer = document.createElement('div');
                this.ganttChartContainer.style.margin = '20px';
                this.ganttChartContainer.style.height = 'calc(100% - 40px)'

                htmlElement.appendChild(this.ganttChartContainer);
                this.ganttChartViewer = new Gantt(element, this.ganttChartSettings);
            }

            this._update(data, this._getganttChartSizeOptions());
        } else {
            while (htmlElement.firstChild)
                htmlElement.removeChild(htmlElement.firstChild);
    ...
    setSize(width, height) {
        ...
        this._update(null, this._getganttChartSizeOptions());
    };
    setSelection(values: Array<Array<any>>) {
        super.setSelection(values);
        this._update(this._getDataSource(), null);
    };
    ...
    _getganttChartSizeOptions() {
        if (!this.ganttChartContainer)
            return {};

        return { size: { width: this.ganttChartContainer.clientWidth, height: this.ganttChartContainer.clientHeight } };
    };
    ...
    _getDataSource() {
        ...
        this.iterateData((dataRow) => {
            ...
            if (this._hasArguments()) {
                for (let i = 0; i < this.getBindingValue('Arguments').length; i++) {
                    var items = values.map((value, index) => { return '"' + bindingValues[index].displayName() + '":' + value }).toString();
                    var json = '{ "arg": "' + dataRow.getDisplayText('Arguments')[i] + '", ' + items + "}";
                    data.push(JSON.parse(json));
                }
            } else {
                data = values.map((value, index) => { return { arg: bindingValues[index].displayName(), val: value }; });
            }
        });
    ...
    _getSeriesSettings() {
        ...
        if (bindingValues.length == 0)
            return undefined;
        var series = [];
        var type = this.getPropertyValue('commonSeriesSettings');
        this.iterateData((dataRow) => {
            var values = dataRow.getValue('Values');
            if (this._hasArguments()) {
                series = values.map((value, index) => {
                    return {
                        valueField: bindingValues[index].displayName(),
                        name: bindingValues[index].displayName(),
                        color: dataRow.getColor("Values")[index], type: type
                    }
                });
            } else {
    ...
    _ensureganttChartLibrary(htmlElement: HTMLElement) {
        if (!Gantt) {
            ...
            textDiv.innerText = "'GanttChart' no puede ser mostrado."
    ...
    _ensureganttChartSettings() {
        const scaleType = this.getPropertyValue('scaleType');       
        if (!this.ganttChartSettings) {
            this.ganttChartSettings = {
                tasks: {
                    dataSource: tasks
                },
                dependencies: {
        ...
        return this.ganttChartSettings;
    }



    _onClick(e) {
        if (!this._hasArguments() || !e.target._label)
            return;
        var row = e.target._label._data;
    ...
    _subscribeProperties() {
        // scaleType
        this.subscribe('scaleType', (type) => this._update(null, { scaleType: type }));
    }


    _update(data?, options?) {
        this._ensureganttChartSettings();

        if (!!this.ganttChartViewer) {
            this.initialize(this.ganttChartSettings.dataSource, this.ganttChartSettings);
    ...
    _updateExportingImage() {
        var svg = this.ganttChartContainer.firstElementChild,
    ...
    _getImageBase64() {
        ...
        canvas.width = this.ganttChartContainer.clientWidth;
        canvas.height = this.ganttChartContainer.clientHeight;
    ...
    initialize(data, options) {
        this.ganttChartViewer = new Gantt(this.element, this._ensureganttChartSettings());
    }
